...
function NftModal({
  ...
  console.log("nftItemssss---", nftItem);
...
const fetchNftBasedOnUserAddress = async ({
  chain,
  address,
}: {
  chain: string;
  ...
  console.log("chain, address", chain, address);
  const data = await fetch(
...
const fetchNftFromUri = async ({ uri }: { uri: string }) => {
...
const NftImageSection = ({ nftData }: any) => {
...
export const NFT = () => {
  ...
  const [fieldValue, selectField] = React.useState({ chain: "", address: "" });
